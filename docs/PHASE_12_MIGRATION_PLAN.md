...
export const useAnalytics = (tenantId: string) => {
  const [state, setState] = useState<AsyncResult<ContactStats, AnalyticsError> | null>(null);
    ...
    try {
      ...
      if (!response.ok) {
        try {
          ...
          return errAsync(createApiError(response.status, `HTTP ${response.status}: ${response.statusText}`));
        }
      }
      ...
      try {
        ...
        // Handle JSON parsing failures for successful responses
        return errAsync(createApiError('PARSE_ERROR', `Failed to parse response JSON: ${String(parseError)}`, {
...
export class ServiceAdapter<T, E> {
  ...
  async executeWithFallback(): AsyncResult<T, E> {
    ...
    return fpResult.match(
      ...
      async (error) => {
        ...
        try {
          const legacyResult = await this.oldService(); // Fallback to old implementation
          return okAsync(legacyResult);
        } catch (legacyError) {
          // Convert legacy error to the same error type as FP service
          return errAsync(error); // Return original FP error or convert legacy error
        }
      }
    );
  }
...
import type { AsyncResult } from 'neverthrow';

interface UseAsyncReturn<T, E> {
  data?: T;
  error?: E;
  state: 'idle' | 'loading' | 'success' | 'error';
  execute: (args: any) => Promise<AsyncResult<T, E>>;
}

export const useAsync = <T, E>(asyncFn: (...args: any[]) => AsyncResult<T, E>): UseAsyncReturn<T, E> => {
  const [state, setState] = useState<'idle' | 'loading' | 'success' | 'error'>('idle');
  const [data, setData] = useState<T | undefined>(undefined);
  const [error, setError] = useState<E | undefined>(undefined);

  const execute = useCallback(async (...args: any[]): Promise<AsyncResult<T, E>> => {
    setState('loading');
    setError(undefined);
    setData(undefined);
